...
Deno.serve(async (req) => {
  ...
  try {
    ...
    requireFields(body, ["elder_id", "kind"]);
    const db = admin();
    let result;
    if (body.kind === "hydration") {
      const { data, error } = await db.from("hydration_logs").insert({ elder_id: body.elder_id, amount_ml: body.amount_ml ?? 250, source: body.source ?? "voice", notes_nl: body.notes_nl, notes_en: body.notes_en }).select().single();
      if (error) throw error;
      result = data;
    } else if (body.kind === "nutrition") {
      const { data, error } = await db.from("nutrition_logs").insert({ elder_id: body.elder_id, meal_label: body.meal_label, description_nl: body.description_nl, description_en: body.description_en, appetite_score: body.appetite_score }).select().single();
      if (error) throw error;
      result = data;
      ...
      const { data, error } = await db.from("vital_signs").insert({ elder_id: body.elder_id, vital_type: body.vital_type, value: body.value, unit: body.unit, reading_source: body.reading_source ?? "manual", device_name: body.device_name, context_notes_nl: body.context_notes_nl, threshold_flag: threshold }).select().single();
      if (error) throw error;
      result = data;
    } else {
